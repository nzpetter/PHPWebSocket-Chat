...
class PHPWebSocket
{
	...
	function wsAddClient($socket,$clientIP,$clientName = NULL)
	{
		...
		$this->wsClients[$clientID] = array($socket,'',self::WS_READY_STATE_CONNECTING,time(),false,0,$clientIP,false,0,'',0,0,$clientName);
	...
	function wsRemoveClient($clientID)
	{
		...
		if(array_key_exists('close',$this->wsOnEvents)) foreach($this->wsOnEvents['close'] as $func) $func($clientID,$closeStatus);
